...
# This function is too large. mainly because the main game loop should be in a separate function (probably a function named `play` of a class named `Game`)
@app.command()
    ...
    for i in range(len(words_to_play)):
        ...
        while not check_completed_word(word_indices, guessed_letters):
            typer.echo(
                "\n"
                + "  ".join(
                    [f"{player['name']}: {player['score']}" for player in players]
                )
            )
            typer.echo(f"Current word: {' '.join(guessed_word)}")
            typer.echo(f"\nGuessed Letters: {' '.join(guessed_letters)}")
            guess = take_guess(player_turn, guessed_letters)
            guessed_word = handle_guess(
                player_turn, guessed_letters, players, word_indices, guess, guessed_word
            )
